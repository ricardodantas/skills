...
Uses `mcr.microsoft.com/playwright:v1.61.0-noble` with `playwright@1.61.0` npm package (versions must match).

To upgrade, bump both to any matching Playwright release: the image tag `vX.Y.Z-noble` and `playwright@X.Y.Z` must be the same version (set `IMAGE` and `PLAYWRIGHT_VERSION` in `browse.js`).
